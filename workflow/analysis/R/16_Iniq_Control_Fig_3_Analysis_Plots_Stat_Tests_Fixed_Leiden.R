...
`%ni%` <- Negate(`%in%`)
setwd("../../../results/control_leiden_res/")

# This script contains code for re-analysis of Figure 3 data (plots and)
# statistical tests with a fixed leiden resolution based on the pre-integration data.

# Essentially, for each method, the leiden resolution was set to a resolution in the control case
# such that the number of clusters was the same as the number of celltypes in this setting. This 
# same resolution was then used for all other cases (ablations, downsampling, etc). 
...
# Make the leiden fix results directory if it doesn't exist 
if (!dir.exists("outs/control_leiden_fix/figures")) {
  dir.create("outs/control_leiden_fix/figures", recursive = TRUE)
}
if (!dir.exists("outs/control_leiden_fix/results")) {
  dir.create("outs/control_leiden_fix/results", recursive = TRUE)
}
...
ggsave(
  "outs/control_leiden_fix/figures/16_pbmc_ds_ablate_allmethod_cluster_number_no_liger.pdf",
  width = 14,
  height = 8,
  device = cairo_pdf
)  
...
ggsave(
  "outs/control_leiden_fix/figures/16_pbmc_ds_ablate_clus_num_celltype_ari_corr_no_liger.pdf",
  width = 14,
  height = 8,
  device = cairo_pdf
)
...
ggscatter(imba_clus_merged, 
  ...
  theme(legend.text = element_text(size = 14)) 
ggsave(
  "outs/control_leiden_fix/figures/16_pbmc_ds_ablate_clus_num_batch_ari_corr_no_liger.pdf",
  width = 14,
  height = 8,
  device = cairo_pdf
)
...
ggsave(
  paste0(
    "outs/control_leiden_fix/figures/16_pbmc_ds_ablate_",
...
CairoPDF(
  "outs/control_leiden_fix/figures/16_marker_gene_pert_pbmc_control_heatmap_no_liger.pdf",
  width = 14, 
  height = 6
)
draw(
...
CairoPDF(
  "outs/control_leiden_fix/figures/16_marker_gene_pert_pbmc_control_heatmap_no_ctype_no_liger.pdf",
  width = 14, 
  height = 6
)
draw(
...
ggsave(
  paste0(
    "outs/control_leiden_fix/figures/16_pbmc_ds_ablate_dge_rankings_top_10_var_genes_no_liger.pdf"
  ),
  width = 14,
  height = 8,
  device = cairo_pdf
...
ggsave(
  paste0(
    "outs/control_leiden_fix/figures/16_pbmc_ds_only_",
...
ggsave(
  paste0(
    "outs/control_leiden_fix/figures/16_pbmc_ablated_only_",
...
fwrite(
  ...
  paste0(
    "outs/control_leiden_fix/results/",
    "16_pbmc_base_clus_num_leiden_aov_results_ctrl_method_celltype_ds_no_liger.tsv"
  ),
  sep = "\t",
  quote = FALSE,
  row.names = FALSE,
...
fwrite(
  ...
  paste0(
    "outs/control_leiden_fix/results/",
    "16_pbmc_base_dge_rank_aov_results_ctrl_gene_method_celltype_ds_no_liger.tsv"
  ),
  sep = "\t",
  quote = FALSE,
  row.names = FALSE,
...
fwrite(
  ...
  paste0(
    "outs/control_leiden_fix/results/",
    "16_pbmc_base_dge_specific_rank_aov_results_ctrl_method_celltype_ds_no_liger.tsv"
  ),
  sep = "\t",
  quote = FALSE,
  row.names = FALSE,
...
fwrite(
  ...
  paste0(
    "outs/control_leiden_fix/results/",
    "16_pbmc_base_dge_rank_coeffs_celltype_ds_method_importance_ranks_no_liger.tsv"
  ),
  sep = "\t",
  quote = FALSE,
  row.names = FALSE,
