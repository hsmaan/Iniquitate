...
def main(h5ad_loc, save_loc, dataset_name, rep):
    ...
    # Determine KNN-cluster-based accuracy for each batch-correction method
    precision_scores = []
    recall_scores = []
    f1_scores = []
    supports = []
    ...
    for adata_sub in adata_method_sub:
        ...
        # One-hot encode X train and test
        enc = OneHotEncoder(handle_unknown="ignore") # ignore unknown categories in test set
        X_train_reshaped = X_train.reshape(-1, 1)
        X_test_reshaped = X_test.reshape(-1, 1)
        enc.fit(X_train_reshaped)
        X_train = enc.transform(X_train_reshaped).toarray()
        X_test = enc.transform(X_test_reshaped).toarray()
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description = "Input and output files for knn-cluster classification summary"
    )
    parser.add_argument(
        "--infile",
        type = str,
