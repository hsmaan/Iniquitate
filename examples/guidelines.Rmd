...
```{r}
features <- SelectIntegrationFeatures(object.list = retina_list)
retina_list <- lapply(retina_list, function(x) {
  x <- ScaleData(x, features = features, verbose = FALSE)
  x <- RunPCA(x, features = features, verbose = FALSE)
})
anchors <- FindIntegrationAnchors(
  object.list = retina_list,
  anchor.features = features,
  reduction = "rpca"
)
retina_integrated <- IntegrateData(anchorset = anchors)

DefaultAssay(retina_integrated) <- "integrated"
retina_integrated <- ScaleData(retina_integrated)
retina_integrated <- RunPCA(retina_integrated, npcs=15)
retina_integrated <- FindNeighbors(retina_integrated, reduction="pca", dims=1:15)
retina_integrated <- FindClusters(retina_integrated, resolution=0.5, algorithm="leiden")
retina_integrated <- RunUMAP(retina_integrated, reduction="pca", dims=1:15)
```
...
```{r}
library(harmony, eval=FALSE)

rm(retina_integrated)
rm(retina_2_sce)
rm(retina_1_sce)
rm(retina_list)
gc()

retina_combined <- merge(retina_1, retina_2)
rm(retina_1)
rm(retina_2)
retina_combined <- NormalizeData(
  retina_combined, normalization.method = "LogNormalize", scale.factor = 10000
)
retina_combined <- FindVariableFeatures(retina_combined, nfeatures = 2000)
all.genes <- rownames(retina_combined)
retina_combined <- ScaleData(retina_combined, features = all.genes)
retina_combined <- RunPCA(retina_combined, features = VariableFeatures(retina_combined))
retina_combined <- RunHarmony(retina_combined, "batch")

retina_combined <- FindNeighbors(retina_combined, reduction="harmony", dims=1:15)
retina_combined <- FindClusters(retina_combined, resolution=0.5, algorithm="leiden")
retina_combined <- RunUMAP(retina_combined, reduction="harmony", dims=1:15)
```

# Plot and visualize the results 
```{r, eval=FALSE}
DimPlot(retina_combined, reduction="umap", group.by="batch")
DimPlot(retina_combined, reduction="umap", group.by="seurat_clusters")
```

# Measure the degree of batch mixing 
```{r, eval=FALSE}
library(bluster)
batch_labels <- retina_combined@meta.data$batch
integrated_cluster_labels <- retina_combined@meta.data$seurat_clusters
a <- 1 - pairwiseRand(batch_labels, integrated_cluster_labels)
a
```
...
cluster_labels_b1_after <- retina_combined@meta.data$seurat_clusters[
  retina_combined@meta.data$batch == "Batch 1"
]
cluster_labels_b2_after <- retina_combined@meta.data$seurat_clusters[
  retina_combined@meta.data$batch == "Batch 2"
]
