...
Lets compare the pre and post-integration unsupervised clustering results: 
```{r, fig.height=8}
library(pheatmap)
...
Lets go ahead and see what the results look like using the UMAP reduction: 
```{r}
DimPlot(pbmc_integrated, reduction="umap", group.by="batch")
DimPlot(pbmc_integrated, reduction="umap", group.by="seurat_clusters")
```

We can already see that the batch mixing might not be as strong here compared to the CCA approach. Lets quantify this:
```{r}
library(bluster)
batch_labels <- pbmc_integrated@meta.data$batch
integrated_cluster_labels <- pbmc_integrated@meta.data$seurat_clusters
...
Lets visualize the results of Harmony integration and quantify batch mixing:
```{r}
library(bluster)
